    ...
    for resource in required_resources:
        try:
            ...
            try:
                ...
                print(f"[FEHLER] NLTK-Download für '{resource}' fehlgeschlagen.")
...
def train_and_save_model():
    """Lädt die Daten aus der DB und trainiert das BERT-Modell neu."""
    global model
    print("[LOG] Starte BERT-Modell-Training...")
    try:
        training_data_list = list(training_data_collection.find({}, {'_id': 0}))
        if not training_data_list or len(training_data_list) < 20: # Mindestanzahl für das Training
            print("[LOG] Nicht genügend Daten für das Training. Mindestens 20 Datensätze erforderlich.")
            model = None
            return

        df = pd.DataFrame(training_data_list)
        df['label'] = df['label'].map({'menschlich': 0, 'ki': 1})
        ...
        print("[LOG] Modell wurde erfolgreich neu trainiert und gespeichert.")
...
def load_model():
    ...
    try:
        ...
        model.load_state_dict(torch.load('distilbert_finetuned.pt'))
        model.to(device)
        model.eval()
        print("[LOG] Feinabgestimmtes BERT-Modell erfolgreich geladen.")
    except FileNotFoundError:
        print("[WARNUNG] Feinabgestimmtes Modell nicht gefunden. Starte Initialtraining...")
        train_and_save_model()
    except Exception as e:
        print(f"[FEHLER] Fehler beim Laden des Modells: {str(e)}")
        model = None
...
# Die anderen Endpunkte (/admin_login, /add_data, /delete_data, /get_data_status) bleiben unverändert.


# HILFSFUNKTIONEN UND ENDPUNKTE HIER EINFÜGEN, DIE UNVERÄNDERT BLEIBEN.
# Sie müssen die ursprünglichen Funktionen und Endpunkte aus dem alten app.py-Skript
# hierher kopieren, um die volle Funktionalität zu erhalten.
